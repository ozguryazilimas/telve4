 ...
 *
 * @author yusuf
 * @param <E> İşlenecek olan entity
 * @param <D> Alt Entity
 */
public class SubDateFilter<E, D> extends Filter<E, D, Date> {
        ...
        if (getValue() != null) {
            switch (getOperand()) {
                ...
                case NotEqual:
                    ...
                    break;
                case Greater:
                    predicates.add(builder.greaterThan(from.get(getAttribute()).get(subattribute), getValue()));
                    break;
                case GreaterOrEqual:
                    predicates.add(builder.greaterThanOrEqualTo(from.get(getAttribute()).get(subattribute), getValue()));
                    break;
                case Lesser:
                    predicates.add(builder.lessThan(from.get(getAttribute()).get(subattribute), getValue()));
                    break;
                case LesserOrEqual:
                    predicates.add(builder.lessThanOrEqualTo(from.get(getAttribute()).get(subattribute), getValue()));
                    break;
                case In:
                    predicates.add(builder.between(from.get(getAttribute()).get(subattribute), getValue(), getValue2()));
                    break;
                case Between:
                    predicates.add(builder.between(from.get(getAttribute()).get(subattribute), getValue(), getValue2()));
                    break;
