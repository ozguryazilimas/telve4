 ...
 * @param <E>
 */
public class DateTimeFilter<E> extends Filter<E, Date, Date> {
    ...
    public DateTimeFilter(SingularAttribute<? super E, Date> attribute, String label,
        FilterOperand operand, DateValueType valueType) {
        super(attribute, label);
    ...
    public void decorateCriteriaQuery(List<Predicate> predicates, CriteriaBuilder builder,
        ...
        if (getValue() != null) {
            switch (getOperand()) {
                case Equal:
                    predicates.add(builder.between(from.get(getAttribute()), getValue(),
                        new LocalDate(getValue()).plusDays(1).toDate()));
                    break;
                case NotEqual:
                    predicates.add(builder.not(builder.between(from.get(getAttribute()), getValue(),
                        new LocalDate(getValue()).plusDays(1).toDate())));
                    break;
                case Greater:
                    predicates.add(builder.greaterThan(from.get(getAttribute()),
                        new LocalDate(getValue()).plusDays(1).toDate()));
                    break;
                case GreaterOrEqual:
                    predicates
                        .add(builder.greaterThanOrEqualTo(from.get(getAttribute()), getValue()));
                    break;
                case Lesser:
                    predicates.add(builder.lessThan(from.get(getAttribute()), getValue()));
                    break;
                case LesserOrEqual:
                    predicates.add(builder.lessThanOrEqualTo(from.get(getAttribute()),
                        new LocalDate(getValue()).plusDays(1).toDate()));
                    break;
                case In:
                    predicates.add(builder.between(from.get(getAttribute()), getValue(),
                        new LocalDate(getValue2()).plusDays(1).toDate()));
                    break;
                case Between:
                    predicates
                        .add(builder.between(from.get(getAttribute()), getValue(), getValue2()));
                    break;
                default:
                    break;
            }
    ...
    public String serialize() {
        ...
        return Joiner.on("::").join(getOperand(),
            getValueType(), getValue() == null ? "null"
                : DateUtils.getDateTimeFormatter().print(new DateTime(getValue())),
            getValueType2(),
            getValue2() == null ? "null"
                : DateUtils.getDateTimeFormatter().print(new DateTime(getValue2())));
    }
